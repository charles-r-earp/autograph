...
impl Model {
    fn new(kind: ModelKind, device: Device, scalar_type: ScalarType) -> Result<Self> {
        match kind {
            ...
            ModelKind::Lenet5 => Ok(Self::Lenet5(Lenet5::new(device, scalar_type)?)),
        }
    }
}
...
impl Layer for Model {
    fn set_training(&mut self, training: bool) -> Result<()> {
        match self {
            Self::Linear(x) => x.set_training(training),
            Self::ConvNet(x) => x.set_training(training),
            Self::Lenet5(x) => x.set_training(training),
        }
    }
    fn parameters_mut(&mut self) -> Result<Vec<ParameterViewMutD>> {
        match self {
            ...
            Self::Lenet5(x) => x.parameters_mut(),
        }
    }
}
...
impl Forward<Variable4> for Model {
    ...
    fn forward(&self, input: Variable4) -> Result<Self::Output> {
        match self {
            Self::Linear(x) => x.forward(input),
            Self::ConvNet(x) => x.forward(input),
            Self::Lenet5(x) => x.forward(input),
        }
    }
}
