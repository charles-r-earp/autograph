...
fn test_conv2d_with_args(input_dim: impl IntoDimension<Dim=Ix4>, outputs: usize, kernel: impl Into2d, use_bias: bool, args: &Conv2dArgs, device: impl Into<Device>) {
  ...
  compare_vectors(&output_vec, &output_true_vec, batch_size, inputs*kh*kw, outputs);
}
...
fn test_conv2d_backward_input_with_args(input_dim: impl IntoDimension<Dim=Ix4>, outputs: usize, kernel: impl Into2d, args: &Conv2dArgs, device: impl Into<Device>) {
  let kernel = kernel.into_2d();
  let device = device.into();
  let input_dim = input_dim.into_dimension();
  let (batch_size, inputs, ih, iw) = input_dim.into_pattern();
  ...
  compare_vectors(&input_grad_vec, &input_grad_true_vec, batch_size, outputs, inputs);
  assert!(
    approx::relative_eq!(&*input_grad_vec, &*input_grad_true_vec, max_relative=0.001),
    "\n{:?}\n !=\n{:?}",
    &input_grad_vec, &input_grad_true_vec
  ); 
}
fn test_conv2d_backward_input(device: impl Into<Device>) {
  let device = device.into();
  test_conv2d_backward_input_with_args([1, 1, 3, 3], 1, [2, 2], &Conv2dArgs::default(), device.clone()); 
  test_conv2d_backward_input_with_args([40, 1, 28, 28], 6, [5, 5], &Conv2dArgs::default(), device.clone()); 
  test_conv2d_backward_input_with_args([40, 6, 24, 24], 16, [5, 5], &Conv2dArgs::default(), device.clone()); 
}
/*#[test]
...
fn test_conv2d_backward_weight_bias_with_args(input_dim: impl IntoDimension<Dim=Ix4>, outputs: usize, use_bias: bool, kernel: impl Into2d, args: &Conv2dArgs, device: impl Into<Device>) {
  let kernel = kernel.into_2d();
  let device = device.into();
  let input_dim = input_dim.into_dimension();
  let (batch_size, inputs, ih, iw) = input_dim.into_pattern();
  ...
  assert!(
    approx::relative_eq!(&*weight_grad_vec, &*weight_grad_true_vec, max_relative=0.001),
    "\n{:?}\n !=\n{:?}",
    &weight_grad_vec, &weight_grad_true_vec
  ); 
  if use_bias {
    ...
    assert!(
      ...
      "\n{:?}\n !=\n{:?}",
      &bias_grad_vec, &bias_grad_true_vec
    ); 
  }
}
fn test_conv2d_backward_weight_bias(device: impl Into<Device>) {
  let device = device.into();
  test_conv2d_backward_weight_bias_with_args([1, 1, 20, 20], 6, false, [5, 5], &Conv2dArgs::default(), device.clone());
  test_conv2d_backward_weight_bias_with_args([1, 6, 20, 20], 16, true, [5, 5], &Conv2dArgs::default(), device.clone());
}
#[cfg(feature="cuda")]
#[test]
...
fn test_max_pool2d_backward_with_args(input_dim: impl IntoDimension<Dim=Ix4>, args: &Pool2dArgs, device: impl Into<Device>) {
  let device = device.into();
  let input_dim = input_dim.into_dimension();
  let (batch_size, inputs, ih, iw) = input_dim.into_pattern();
  ...
  let input_grad_vec = {
    ...
    let mut input_grad = Tensor::zeros(&device, input_dim);
    let output_grad = Tensor::from_shape_vec(&device, output_dim, output_grad_vec.as_slice());
    max_pool2d_backward(&input, &mut input_grad, args, workspace.as_ref(), &output_grad);
