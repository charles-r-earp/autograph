...
impl<X, Y, Fut: Future<Output = Result<X>>, F: Clone + Fn(X) -> Result<Y>> Future
    for FutureOkMap<Fut, F>
{
    type Output = Result<Y>;
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let f = self.f.clone();
        let fut = unsafe { self.map_unchecked_mut(|x| &mut x.fut) };
...
impl<
        'a,
        X,
        XFuture: Future<Output = Result<X>>,
        A: Dataset<Item = X, Future = XFuture>,
        Y,
        F: Clone + Fn(X) -> Result<Y>,
    > Dataset for Map<'a, A, F>
{
    type Item = Y;
