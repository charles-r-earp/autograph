...
use ndarray::{ArrayView, ArrayView1, ArrayView4};
use std::{
    env,
    fs::{DirBuilder, File},
    io::Read,
...
impl Mnist {
    ...
    pub fn new() -> Self {
        ...
        let train_images = {
            ...
            assert_eq!(decoder.read_i32::<BigEndian>().unwrap(), 28);
            decoder.read_to_end(&mut train_images).unwrap();
        ...
        let train_labels = {
            ...
            assert_eq!(decoder.read_i32::<BigEndian>().unwrap(), 60_000);
            decoder.read_to_end(&mut train_labels).unwrap();
        ...
        let test_images = {
            ...
            assert_eq!(decoder.read_i32::<BigEndian>().unwrap(), 28);
            assert_eq!(decoder.read_i32::<BigEndian>().unwrap(), 28);
            decoder.read_to_end(&mut test_images).unwrap();
        ...
        let test_labels = {
            ...
            assert_eq!(decoder.read_i32::<BigEndian>().unwrap(), 2_049);
            assert_eq!(decoder.read_i32::<BigEndian>().unwrap(), 10_000);
            decoder.read_to_end(&mut test_labels).unwrap();
    ...
    pub fn train<'a>(
        &'a self,
        batch_size: usize,
    ) -> impl Iterator<Item = (ArrayView4<'a, u8>, ArrayView1<'a, u8>)> + 'a {
        self.train_images
            .as_slice()
